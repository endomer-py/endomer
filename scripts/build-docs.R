...
output <- if (length(args)) args[[1]] else file.path(workspace, "artifacts/endomer-engih-release/sites/r")
...
if (packageVersion("labeler") < "0.11.0") stop("Install labeler >= 0.11.0 first")
...
library(endomer)
if (packageVersion("endomer") != as.character(read.dcf(file.path(pkg, "DESCRIPTION"))[1, "Version"])) stop("Wrong source package version")
Sys.setenv(R_LIBS = paste(.libPaths(), collapse = .Platform$path.sep))
...
# Reuse the package's existing icons in English as well. Generating new icons
# would upload the same logo to a remote service on each translated build.
dir.create(file.path(stage,"pkgdown"),showWarnings=FALSE)
file.copy(file.path(pkg,"pkgdown/favicon"),file.path(stage,"pkgdown"),recursive=TRUE,overwrite=TRUE)
desc::desc_set(URL = paste0(read.dcf(file.path(stage, "DESCRIPTION"))[1, "URL"], ", https://endomer-py.github.io/endomer/en/"), file = file.path(stage, "DESCRIPTION"))
pkgdown::build_site_github_pages(pkg = stage, dest_dir = file.path(output, "en"), clean = file.exists(file.path(output, "en/pkgdown.yml")),
  install = FALSE, new_process = FALSE, examples = TRUE)
writeLines("endomer docs output", marker)
jsonlite::write_json(list(package = "endomer", version = as.character(read.dcf(file.path(pkg, "DESCRIPTION"))[1, "Version"]), languages = c("es", "en"),
